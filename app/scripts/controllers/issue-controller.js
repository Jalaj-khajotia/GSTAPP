...
angular.module('sbAdminApp')
    .controller('IssueCtrl', function ($scope, $http, $location, $stateParams, $localStorage, $cookieStore, Library) {
            ...
            Library.getAllIssueForUser(userID).then(function (userIssuedData) {
                ...
                function callback(element, index, array) {
                    ...
                    for (var i = 0; i < element.Issue_books.length; i++) {
                        ...
                        if (element.Issue_books[i].pending) {
                            pendingCount++;
                        }
        ...
        $scope.ReturnBooks = function () {
            Library.updateReturnBooks(selectedBooks);
            $('#modal1').closeModal();
            $location.path('/dashboard/returns');
        };

        $scope.addOrRemoveBook = function (index) {
            var position = selectedBooks.indexOf($scope.books[index]);
            if (position >= 0) {
                selectedBooks.splice(position, 1);
            }
            else {
                selectedBooks.push($scope.books[index]);
            }
